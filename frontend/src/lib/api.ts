 ...
 * This is the single boundary between TypeScript and Rust — every command
 * call in the application must go through here.
 *
 * Design rules:
 * - Functions throw on error (the string the backend returns becomes the
 *   Error message) so callers use try/catch naturally.
 * - No raw passwords are returned; copy_password sends directly to the
 *   clipboard via the backend without the value ever reaching JS.
 * - All shapes mirror the Rust structs exactly (snake_case field names as
 *   serialized by serde).
 */
 ...
 * Use `copyPassword` for entry passwords to avoid IPC exposure.
 */
 ...
 * `seconds` must be between 10 and 900 (inclusive).
 */
 ...
 *
 * Returns an unsubscribe function — call it in a React cleanup effect to
